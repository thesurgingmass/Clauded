...
namespace fs
{
    ...
    void FMOperator::setFixedFrequency(bool useFixed, double fixedHz)
    {
        fixedFrequencyMode = useFixed;
        fixedFrequencyHz = fixedHz;
    }

    float FMOperator::renderSample(double baseFrequencyHz, float phaseModulation)
    {
        const double frequency = fixedFrequencyMode
            ? fixedFrequencyHz
            : baseFrequencyHz * ratioCoarse * semitonesToRatio(ratioFineSemitones);
